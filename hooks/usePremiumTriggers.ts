...
export function usePremiumTriggers(): {
  ...
  const pendingTrigger = useMemo<PremiumTrigger | null>(() => {
    ...
    // Ordem de prioridade: dos mais qualificados pros menos
    // (quanto mais engajado o usuário, maior a chance de converter)

    // 1. 30+ dias de streak → propaganda forte (power user)
    if (streak >= 30 && !shownPrompts.includes('streak-30-power')) {
      return {
        id:       'streak-30-power',
        emoji:    '👑',
        headline: 'Você é um tutor excepcional!',
        subtitle: `${streak} dias de rotina perfeita. Que tal backup em nuvem pra nunca perder esse progresso?`,
        feature:  'cloud-sync',
    ...
    if (premium.daysSinceFirstOpen >= 7 && !shownPrompts.includes('seven-days-family')) {
      return {
    ...
    if (weightHistory.length >= 3 && !shownPrompts.includes('three-weights-plans')) {
      return {
    ...
    if (actionHistory.length >= 50 && !shownPrompts.includes('fifty-logs-backup')) {
      return {
        id:       'fifty-logs-backup',
        emoji:    '💾',
        headline: `${actionHistory.length} registros preciosos`,
    ...
    if (premium.daysSinceFirstOpen >= 14 && !shownPrompts.includes('two-weeks-gentle')) {
      return {
        id:       'two-weeks-gentle',
        emoji:    '🎁',
